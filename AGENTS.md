...
10. Work is tracked as GitHub issues, managed with `gh`. Every issue names a work shape and a recommended starting model and effort for both agents, following [model and effort](docs/model-effort.md). Take one issue per session: add the `agent:claude` or `agent:codex` label and comment the settings you start with. When closing, comment what was verified and the settings actually used. The repository is public, so issues carry code discussion only, as the agent channel does. When a session runs long or its remaining work fits a cheaper setting, stop at a committed point, comment a checkpoint on the issue, and recommend a fresh session (user, 2026-09-14).
