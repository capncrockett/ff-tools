...
1. Check `git status --short --branch` and run `pnpm run doctor`. Preserve user edits.
...
6. Run `pnpm run verify -- --e2e` before every commit and handoff. The user asked on 2026-09-13 for e2e runs early and often, not only for visible UI changes. Do not bypass checks or hooks.
...
10. Work is tracked as GitHub issues, managed with `gh`. Every issue names a work shape and a recommended starting model and effort for both agents, following [model and effort](docs/model-effort.md). Take one issue per session: confirm your actual model and effort with the user rather than assuming it, add the `agent:claude` or `agent:codex` label, and comment the settings you start with, flagging immediately if they differ from the issue's recommendation. When closing, comment what was verified, the settings actually used, and whether the recommended setting would have been enough given what the work turned out to need. The repository is public, so issues carry code discussion only, as the agent channel does. When a session runs long or its remaining work fits a cheaper setting, stop at a committed point, comment a checkpoint on the issue, and recommend a fresh session (user, 2026-09-14).

Use `rg` for exact names, paths, keys, and exhaustive matches. When the wording or location is unknown, or the answer depends on relationships across files, use the local zvec-grep index for one focused semantic search and verify the result with `rg` or direct file reads. Run `pnpm run search:index` to create or refresh the ignored local index. Do not use remote embeddings for this repository without explicit user approval.

## Data boundaries

- `prisma/dev.db` is the user's real tracker database. It is backed up automatically outside the repository ([backups](docs/backups.md)), but a backup is a recovery path, not permission to risk the data. Never delete, prune, or commit backups. Run tests only through `pnpm test`, `pnpm run verify`, or `pnpm run test:e2e`, which create throwaway databases. Never run jest or another test runner directly; pass file arguments through the wrapper (`pnpm test -- tests/unit/foo.test.ts`). Never reset, push, migrate, seed, capture into, or delete the local database unless the user asks. To inspect it, use only `pnpm run db:query`, which opens it read-only; the user wants agents to have that access (2026-09-13). On 2026-09-13 a direct jest run wiped it (S14). Jest and test-mode config guards now refuse, but do not rely on them.
