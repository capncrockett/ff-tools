...
2. **Player values** shows one row per player with **Dynasty GM** and **DTC** columns. Each value includes growth from its own starting value. Click either value for that source's dated history. Search by name, filter by position, or sort growth for a specific source. Use the player's single **Record entry** button, then choose a source for the acquisition cost and return.
