...
7. `prisma` and `@prisma/client` pinned to exact `7.10.0`, plus `@prisma/adapter-better-sqlite3@7.10.0` and `better-sqlite3@^12.11.1` (GH issue #4), each passing `npm run verify -- --e2e` (122 tests across 23 suites, both builds, 17 Chromium checks).
   - `prisma-client` generator outputs to `src/server/generated/prisma`; all ~15 `@prisma/client` imports now point there. The folder is excluded from ESLint (`globalIgnores`), Prettier (`.prettierignore`), Jest coverage (`coveragePathIgnorePatterns`), and `.gitignore`. `better-sqlite3` does ship its own types, but `@prisma/adapter-better-sqlite3`'s peer does not; added `@types/better-sqlite3` as a dev dependency after `tsc` asked for it.
   - `prisma.config.ts` imports `resolvedDatabaseUrl` directly from `src/server/config.ts` with no path/loader friction; that import's side effect (dotenv-then-default) supplies the URL with no `DATABASE_URL` set, confirmed against a bare `npm run prisma:generate`.
   - **Dates:** `db.ts` and the new `tests/testPrismaClient.ts` factory both construct `new PrismaBetterSqlite3({ url }, { timestampFormat: 'unixepoch-ms' })`. `tests/unit/prisma.timestamps.test.ts` writes a row through Prisma and reads the raw column back with a direct `better-sqlite3` connection; confirmed it fails (string, not number) against the adapter's `iso8601` default before the fix.
   - **Paths:** `resolvedDatabaseUrl()` (next to `databaseFile()` in `src/server/config.ts`) is now the one function that turns a possibly-relative `DATABASE_URL` into an absolute `file:` URL, feeding the adapter, `prisma.config.ts`, and the test factory alike. `tests/unit/config.test.ts` confirmed it fails against a naive passthrough before the fix.
   - The four tests calling `new PrismaClient()` directly now use `tests/testPrismaClient.ts`'s `createTestPrismaClient()`. Two more inline reconnections in `providers.run.test.ts` (`const other/restarted = new PrismaClient()`, beyond the four call sites already known) needed the same swap.
   - `withSqliteFile` in `backup.ts` now wraps `better-sqlite3` directly (`{ fileMustExist: true, ...options }`); its two callers (`VACUUM INTO`, `integrity_check`) are unprotected as before. `databaseQuery.ts` calls it with `{ readonly: true }` plus `db.pragma('query_only = ON')`, restoring a true OS-level read-only open. Manually confirmed against a scratch database: `db:query` reads, a write and a missing path are both refused.
   - **Two findings outside the plan, both root-caused and fixed, not worked around:**
     - Prisma 7's `prisma-client` generator types collapse to `any` everywhere under `moduleResolution: "NodeNext"` (confirmed with an isolated repro: identical code typechecks correctly under `"bundler"`). Prisma's own v7 docs recommend `"module": "ESNext", "moduleResolution": "bundler"`. Changed `tsconfig.server.json` to match (it previously overrode the root config to NodeNext); confirmed `dist/server`'s emitted JS is unaffected (plain ESM `import` with the `.js` extensions the source already writes).
     - The generated client uses top-level `import.meta.url`, which ts-jest cannot down-level to CommonJS, and `scripts/test.mjs` ran Jest without `--experimental-vm-modules`. Added that flag. Real ESM mode then exposed two more spots needing fixes: Jest's `jest` global isn't auto-injected under `--experimental-vm-modules`, so 7 test files now `import { jest } from '@jest/globals'`; and `jest.spyOn`/`jest.mocked` can't replace a real ES module's own exported binding (read-only), so `sync.ts`'s `sourceConfigured` function became a `sourceConfig.isConfigured` object property (call sites in `captureWorker.ts` updated) purely so the existing capture-worker tests can still mock it.
   - **Not done:** the plan asked to also manually strip `databaseQuery.ts`'s `readonly`/`query_only` protections once to reconfirm the existing "write hidden in `WITH`" test fails without them. The session's own safety classifier refused that edit as weakening a security boundary, even temporarily reverted. That specific manual strip-and-revert is unverified this session; the behavior is otherwise covered by the passing automated regression and by `readonly: true` being a `better-sqlite3`-level open flag SQLite itself enforces.
8. `dc6d6e5` **UI:** React 19.3.0, Vite 8.3.0 with `@vitejs/plugin-react` 6.1.1, Tailwind 4.3.3 with `@tailwindcss/vite`, DaisyUI 5.7.38 (GH issue #5), passing `npm run verify -- --e2e` (122 tests across 23 suites, both builds, 17 Chromium checks).
   - `useRef<ReturnType<typeof setTimeout>>()` in `HelpTip.tsx` now takes an explicit `| undefined` initial argument; it was the only no-argument `useRef` in `src/web`.
   - `tailwind.config.ts` and `postcss.config.cjs` are deleted. `styles.css` opens with `@import 'tailwindcss'; @plugin 'daisyui'; @plugin 'daisyui/theme' { name: 'dynasty'; default: true; ... }`, carrying the same hex values the old config had. `vite.config.ts` adds the `@tailwindcss/vite` plugin and (unrelated cleanup, surfaced by a new "native config loader" build warning) swaps `__dirname` for `import.meta.dirname`. Unused `postcss`/`autoprefixer` dependencies removed; nothing else referenced them.
   - DaisyUI 5 renames: `tabs-boxed` -> `tabs-box` (`ValueTracker.tsx`, `styles.css`); `tab-active` is unchanged. `input-bordered`, `select-bordered`, `textarea-bordered`, `file-input-bordered` are removed everywhere they appeared (`ValueTracker.tsx`, `TrackerGuide.tsx`) since v5 borders those elements by default and the modifier class no longer exists.
   - **One real regression, found and fixed by the before/after screenshot comparison the issue asked for:** DaisyUI 5's `.input`/`.select`/`.textarea`/`.file-input` now ship `width: clamp(3rem, 20rem, 100%)`, a specified width that wins over the flex-column stretch the "Find a player" search box relied on to fill its row. Added `.filters .search-field input { width: 100%; }` in `styles.css` (specificity beats DaisyUI's single-class rule regardless of source order). Confirmed by re-diffing screenshots before/after the fix.
   - Everything else compared pixel-identical across all 9 captured states (empty, market, trends, portfolio, detail modal, import modal, guide expanded, and both at 375px mobile) once one artifact was ruled out: the detail-modal screenshot first showed the new dialog fade/scale-in transition mid-flight (page content bleeding through, duplicated text) -- a screenshot-timing artifact from capturing before DaisyUI 5's `<dialog>` open animation settled, not a rendering bug; re-captured after a short wait and it matched exactly.
   - Two cosmetic, library-driven differences left as-is, not code bugs: the native `<input type="file">` button reads "Choose File" instead of "CHOOSE FILE" (DaisyUI 5 dropped the `text-transform: uppercase` it put on `::file-selector-button`); nothing in our CSS ever styled that native pseudo-element. No other `.btn`, `.badge`, `.modal-box`, or `.table` override needed reconciling; the existing rules in `styles.css` still target the right selectors and DaisyUI 5 renders the `dynasty` theme's colors the same way.
9. **Land.**
