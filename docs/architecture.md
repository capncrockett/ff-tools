...
Only the owned QB/RB/WR/TE roster is captured today. The persisted Sleeper check supplies both providers with canonical roster identities. A second provider capture within the hour reuses saved roster state while applying any newly available values. Under the user-confirmed 2026-09-13 rule, a player absent from the validated DTC position exports or Dynasty GM player catalog is worth zero on that source. Such observations use `absent:sleeper:<id>` for provenance and keep the same canonical player, scoring context, and capture time. A later listed value continues the same series. Provider notes are stored in the sync-run message and returned with the capture result. Ambiguous identities, incomplete metadata, and failed captures never become zeros. Dynasty GM requires a rendered value for any roster player present in its catalog; if the Sleeper check is unavailable, it preserves its existing capture behavior without synthesizing unlisted players. Existing history is not backfilled. Departed players retain their history after their tracked acquisition is closed; whole-market scouting is future scope.
