...
## Confirmed strict player matching (2026-09-14)

The user confirmed that Sleeper is the source of truth for birth dates, and chose strict matching over two looser options after seeing the first real results. Dynasty GM and DTC dates only confirm a unique name-and-position match. A provider birth date or age that disagrees with Sleeper's, even by days, or that is missing, keeps the row in review; it never links automatically and never changes Sleeper's date. In the first run, 61 of 65 disagreeing Dynasty GM birth dates were within 18 months of Sleeper's, and all 21 disagreeing DTC ages were within 2 years. The user declined linking those and the roughly 400 rows with no provider date at all. Rostered players in that state are resolved by hand through the planned review screen.
