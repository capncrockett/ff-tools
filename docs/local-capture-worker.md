...
The optional local worker captures each configured provider at 04:00 Pacific while the computer is awake and the process is running. It uses the same database, credentials, saved browser sessions, DTC half-PPR validation, and one-hour reservation as the app and manual CLI. Starting the app or reloading a page never starts the worker.
...
`capture:worker` stays in the foreground and checks eligibility once per minute. Keep this terminal running overnight. Ctrl+C requests shutdown, lets an active capture finish and close its browser, and skips the next provider. It is not installed as a Windows startup task or hosted service. The built equivalent is `node dist/server/jobs/captureWorker.js` after `npm run build`.
