...
The worker applies the same player rules as manual captures. A player DTC does not list receives zero on DTC after a successful validated capture. A Dynasty GM player that cannot be matched fails the capture instead. A failed or incomplete capture preserves existing values. Scheduling does not change the accepted Dynasty GM PPR approximation.
