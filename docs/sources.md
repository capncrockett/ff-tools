...
When a canonical Sleeper roster is available, the adapter also checks it against the validated initialization player catalog. A roster player absent from that catalog is valued at zero under the user's rule. A player present in the catalog but missing from the rendered roster is an incomplete capture, not a zero. Ambiguous name-plus-position matches fail closed, and so does a catalog that matches none of the owned players, since that indicates changed names rather than unlisted players. If the optional Sleeper check is unavailable, only verified provider rows are saved and the source status flags the roster check.
