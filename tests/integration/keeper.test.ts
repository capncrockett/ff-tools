...
describe('GET /api/sleeper-tools/keeper-data', () => {
  ...
  it('assembles keeper data from Sleeper endpoints', async () => {
    ...
    nock(base)
      ...
      .reply(200, [{ league_id: leagueId, name: 'Grundle League', season: year }])
    // rosters
    nock(base)
      .get(`/v1/league/${leagueId}/rosters`)
      .reply(200, [{ owner_id: 'owner1', players: ['ply1', 'ply2'] }])
    // users in league
    nock(base)
      .get(`/v1/league/${leagueId}/users`)
      .reply(200, [{ user_id: 'owner1', username: 'owner1', display_name: 'Owner One' }])
    // players map
    nock(base)
      .get(`/v1/players/nfl`)
      .reply(200, {
    ...
    const res = await request(app).get('/api/sleeper-tools/keeper-data').expect(200)
